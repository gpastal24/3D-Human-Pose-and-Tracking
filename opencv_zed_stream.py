...
from general_utils import set_message_multi, send_message_multi, consumer_class
import logging
...
if args.usb:
    ...
    def get_frames(frames):
        return [np.split(frame, 2, axis=1)[0] for frame in frames]
else:
...
VP = VoxelMethod(config, device=args.device, transform=transformss)
# print(VP.transform)
resize_transform = torch.as_tensor(
    VP.resize_transform, dtype=torch.float, device=VP.device
)
...
VP.model.to(device=VP.device).eval()
VP.model.backbone.to(device=VP.device).eval()
if args.trt:
    ...
    logger.info("Loaded TRT weights")
...
def byte_track_fun(pose_dict, final_pose_cpu, props):
    ...
    for prop in props.numpy():
        bbox = [prop[0] - 500, prop[1] - 500, prop[0] + 500, prop[1] + 500, 1]
        a = np.asarray(bbox)
        a[:4] = a[:4] / 10.0
        bboxes.append(a)

    online_targets = bt_tracker.update(np.asarray(bboxes), space_size, space_size)
    online_targets_for_viz = online_targets
    ...
    if online_tlwhs:
        iou = torchvision.ops.box_iou(
            torch.from_numpy(np.asarray(bboxes)[:, :4]),
            torch.from_numpy(np.asarray(online_tlwhs)),
        )
        for ii in iou:
...
if args.visualize_debug:
    def visualize():
        ...
        for ind, img in enumerate(imgs_drawn):
            for i, (pt, pid, bbox_) in enumerate(
                zip(points_proj[ind], t_ids, bboxes_2d[ind])
            ):
        ...
        if args.save_video:
            for i, img in enumerate(imgs_drawn):
                video_writers[i].write(img)
            
            if args.use_bytetrack:
                bird_eye_view = create_bird_eye_view(online_targets_for_viz, space_size, VP.space_center)
                video_writers[-1].write(bird_eye_view)

else:
    def visualize():
        pass

def create_bird_eye_view(online_targets, space_size, space_center):
    width, height = int(space_size[0]), int(space_size[1])
    bird_eye_view = np.ones((height, width, 3), dtype=np.uint8)*255

    offset_x = space_center[0] / 10.0 - width / 2
    offset_y = space_center[1] / 10.0 - height / 2
    ...
    for t in online_targets:
        ...
        x1_shifted = x1 - offset_x
        y1_shifted = y1 - offset_y

        cv2.rectangle(bird_eye_view, (int(x1_shifted), int(y1_shifted)), (int(x1_shifted+w), int(y1_shifted+h)), (255, 0, 0), 2)
        cv2.putText(bird_eye_view, str(tid), (int(x1_shifted), int(y1_shifted)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1)
...
if __name__ == "__main__":
    # COnnections
    if args.save_video:
        output_dir = "output_videos_D2.2_joints"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        video_writers = []
        for i in range(args.cams_n):
            video_writers.append(cv2.VideoWriter(os.path.join(output_dir, f'cam_{i}.mp4'), fourcc, 30.0, (1280, 720)))
        
        if args.use_bytetrack:
            video_writers.append(cv2.VideoWriter(os.path.join(output_dir, 'bird_eye_view.mp4'), fourcc, 30.0, (int(VP.space_size[0]/10), int(VP.space_size[1]/10))))
    ...
    if args.enable_rabbitcnc:
        ...
        [r.start() for r in rabbit_proc]
        if args.inside:
            rb_1 = consumer_class(
                host_name, USER, "tracker", "cloud_track", callback, "corrections.track"
            )  # c1()
            rb_2 = consumer_class(
                host_name,
                USER,
                ...
                "cloud_track_del",
                callback,
                bind_key="corrections.track_del",
            )  # c1()
            rb_3 = consumer_class(
                host_name,
                USER,
            ...
            rb_1 = consumer_class(
                ...
                "cloud_track_out",
                callback,
                bind_key="corrections.track_out",
            )  # c1()
            rb_2 = consumer_class(
                host_name,
                USER,
                ...
                "cloud_track_del_out",
                callback,
                bind_key="corrections.track_del_out",
            )  # c1()
            rb_3 = consumer_class(
                host_name,
                USER,
    ...
    if args.video:
        ...
        files = sorted(glob.glob("videos_long/*.mp4"))
    ...
    tstart = time.time()
    space_size = VP.space_size[:2] / 10.0
    ...
    while True:
        ...
        if sum(rets) == args.cams_n:
            ...
            all_input = [i.to(device=VP.device) for i in final_images]
            ...
            bboxes_sent = [alter_bbox(np.asarray(b)) for b in bboxes_2d]
            points_proj_sent = [p[:, :, :2] / 2 for p in points_proj]

            if args.enable_rabbitcnc:
                msgBufferSet1.put(
                    [
                        [],
                        bboxes_sent[0].tolist(),
                        points_proj_sent[0].tolist(),
                        t_ids,
                        affs,
                        left_images[0],
                    ]
            ...
            # json_name = f"json_outs/frame_{str(counter).zfill(5)}.json"
            # with open(json_name,"w+") as  f:
            #     json.dump(annos[0],f,cls=NumpyArrayEncoder)
    ...
    if args.enable_rabbitcnc:
        [r.terminate() for r in rabbit_proc]
        [r.kill() for r in rabbit_proc if r.is_alive()]

        [d.kill() for d in d3_list]
        [d.stop() for d in d3_list]
        [d.join() for d in d3_list]
